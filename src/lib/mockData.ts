...
export interface VaccineRecord {
  ...
  name: string;
  date?: string;
  ageRecommended: number; // days from birth
  dose: string;
  lot?: string;
  location?: string;
...
export interface PlaceholderTemplate {
  ...
  icon: string;
  description: string;
  templateType: 'mesversario' | 'primeira-vez' | 'consulta' | 'vacina' | 'medida' | 'carta' | 'nota' | 'evento' | 'arte';
  ageRangeStart: number; // in days from birth
  ageRangeEnd?: number; // optional, if null = any time after start
  isCompleted: boolean;
  momentId?: string; // if completed, reference to the moment
  allowMultiple?: boolean; // can have multiple entries
}
