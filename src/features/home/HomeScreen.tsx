...
function StatWidget({ title, icon, accent, highlight, description, onClick }: StatWidgetProps) {
  return (
    <motion.button
      whileTap={{ scale: 0.97 }}
      onClick={onClick}
      className="group relative overflow-hidden rounded-3xl border border-transparent bg-card/80 p-4 text-left shadow-soft transition-all hover:-translate-y-0.5"
      style={{ boxShadow: `0 22px 38px -24px ${accent}33` }}
    >
      <div
        ...
        style={{
          background: `linear-gradient(135deg, ${accent}1a 0%, transparent 50%, ${accent}10 100%)`,
        }}
      />
      <div className="relative flex items-start justify-between gap-2">
        <div
          className="flex h-11 w-11 items-center justify-center rounded-xl border border-white/40 bg-white/70 text-base shadow-sm backdrop-blur-sm dark:border-white/10 dark:bg-white/5"
          style={{ color: accent }}
        >
          {icon}
        </div>
    ...
    <div className="pb-24">
      ...
      <div className="mx-auto max-w-2xl px-4 pt-6">
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          {quickActions.map(action => (
            <motion.button
              ...
              className="group flex h-14 items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-primary/15 via-primary/10 to-transparent text-sm font-medium text-primary transition-all hover:-translate-y-0.5 hover:from-primary/20 hover:via-primary/15"
            >
              {action.icon}
              {action.label}
            </motion.button>
        ...
        <motion.section
          ...
          className="mt-6 rounded-3xl border border-border/60 bg-card/90 shadow-soft transition-all"
        >
          <button
                ...
                <div className="space-y-3 px-4 py-4">
                  {chapterSummaries.map(summary => (
                    <button
                      ...
                      className="group flex w-full items-start gap-3 rounded-2xl border border-transparent bg-card/70 p-3 text-left shadow-soft transition-all hover:-translate-y-0.5 hover:bg-card"
                    >
                      <div
                  ...
                  <button
                    ...
                    className="h-11 w-full rounded-xl border border-border/60 text-sm text-muted-foreground transition-colors hover:bg-muted/50"
                  >
            ...
            {upcomingMilestones.length === 0 ? (
              ...
              upcomingMilestones.map(item => (
                <motion.button
                  ...
                  className="group flex w-full items-center gap-3 rounded-2xl border border-transparent bg-card/85 px-3 py-3 text-left shadow-soft transition-all hover:-translate-y-0.5 hover:bg-card"
                >
            ...
            <div className="-mx-4 mt-3 overflow-x-auto pb-1">
              <div className="flex min-w-full gap-3 px-4">
                {storyCards.map(card => (
                  <div
                    key={card.id}
                    className="min-w-[240px] shrink-0 rounded-3xl border border-transparent bg-card/85 p-4 shadow-soft transition-all hover:-translate-y-0.5"
                  >
