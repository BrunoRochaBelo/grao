...
export function NotificationsSettingsScreen({ onBack }: NotificationsSettingsScreenProps) {
  ...
  const [galleryView, setGalleryView] = useState<'timeline' | 'grid'>('timeline');
  ...
  return (
    <div className="pb-24 px-4 pt-6 max-w-2xl mx-auto">
      <div className="flex items-center mb-4">
        ...
        <h1 className="text-xl font-medium text-center flex-1">Notificacoes e preferencias</h1>
      </div>
      ...
      <div className="space-y-8">
        <section className="space-y-4">
          ...
          <div className="rounded-xl border border-border bg-card p-4 flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-foreground">Receber notificacoes</p>
          ...
          <div className="rounded-xl border border-border bg-card p-4 space-y-3">
            <Label className="text-sm font-medium text-foreground">Frequencia</Label>
            <RadioGroup value={frequency} onValueChange={setFrequency} className="grid gap-2">
              {[
          ...
          <div className="rounded-xl border border-border bg-card p-4 space-y-2">
            ...
            {[
              ...
              >
                ...
                <Switch
                  checked={categorySettings[item.key as keyof typeof categorySettings]}
                  onCheckedChange={() => handleCategoryToggle(item.key as keyof typeof categorySettings)}
        ...
        <section className="space-y-4">
          ...
          <div className="rounded-xl border border-border bg-card p-4 space-y-4">
            <div className="space-y-2">
              <Label>Tema</Label>
              <Select value={themePreference} onValueChange={(value) => setThemePreference(value as typeof themePreference)}>
                <SelectTrigger>
            ...
            <div className="rounded-lg border border-border bg-muted/40 px-3 py-2 flex items-center justify-between">
              <div>
                <p className="text-sm text-foreground font-medium">Backup automatico</p>
                <p className="text-xs text-muted-foreground">Sincronize em nuvem sempre que abrir o app.</p>
              </div>
              <Switch checked={autoBackup} onCheckedChange={setAutoBackup} aria-label="Backup automatico" />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select value={galleryView} onValueChange={(value) => setGalleryView(value as typeof galleryView)}>
                <SelectTrigger>
            ...
            <div className="rounded-lg border border-border bg-muted/40 px-3 py-2 flex items-center justify-between">
              <div>
                <p className="text-sm text-foreground font-medium">Mostrar dicas contextuais</p>
                <p className="text-xs text-muted-foreground">Sugestoes breves durante o uso do app.</p>
              </div>
              <Switch checked={showTips} onCheckedChange={setShowTips} aria-label="Mostrar dicas contextuais" />
            </div>
