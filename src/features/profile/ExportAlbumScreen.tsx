...
export function ExportAlbumScreen({ onBack }: ExportAlbumScreenProps) {
  ...
  const [exportType, setExportType] = useState<"full" | "chapter" | "series">(
    "full"
  );
  ...
  const handleGeneratePdf = () => {
    ...
    setTimeout(() => {
      ...
      toast.success(
        "Exportacao iniciada. Avisaremos quando o PDF estiver pronto."
      );

      setTimeout(() => {
        toast.success("PDF disponivel para download!", {
          description:
            "Confira a central de notificacoes para abrir o arquivo.",
        });
      }, 1500);
    }, 1800);
  };
    ...
    <div className="pb-24 px-4 pt-6 max-w-2xl mx-auto">
      ...
      <div className="space-y-6">
        <div className="bg-primary/5 border border-primary/20 rounded-lg p-4 flex items-start gap-3 text-sm text-muted-foreground">
          ...
          <p>
            Gere um PDF com a curadoria do album do bebe. Escolha todo o album,
            um capitulo especifico ou uma serie recorrente como os mesversarios.
          </p>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card p-4 space-y-2 text-sm">
          ...
          {exportType === "series" && selectedSeries && (
            <p className="text-muted-foreground">
              Serie:{" "}
              {
                seriesOptions.find((series) => series.id === selectedSeries)
                  ?.label
              }
            </p>
          )}
          <p className="text-muted-foreground">
            Tema: {useDarkTheme ? "Escuro" : "Claro"} • Rascunhos:{" "}
            {includeDrafts ? "Sim" : "Nao"} • Tags:{" "}
            {includeTags ? "Sim" : "Nao"}
          </p>
        </div>
      </div>
